...
<!-- trying to get setting from pouch. 2019-03-03 -->
  ...
  <div class="localaapp">
    ...
    <div>- </div>
    <div> {{ couchurl }} ,  {{ couchurl2 }}, {{ this.syncurl.synca }}  </div>
...
export default {
  ...
  created() {
    ...
    // '_local/aapp'.get('1').then(function (doc) {
    //   this.$pouch.sync('maindb', doc.synca)
    // }).catch(function () {
    //   // console.log(err);
    ...
    this.$pouch.get("maindb", "_local/syncu").then((data) => {
       this.syncurl = data.syncu;
    });
    ...
    // this.$pouch.sync('maindb', this.syncurl);
